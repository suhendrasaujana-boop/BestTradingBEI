...
def add_indicators(df):
    ...
    # ATR (Average True Range) untuk stop loss
...
def calculate_score(df):
    """Hitung skor sinyal (0-100) dengan bobot lebih akurat"""
    ...
    # 2. RSI (20 poin)
    if last['rsi'] < 30:  # Oversold -> Buy signal
        score += 20
    elif last['rsi'] > 70:  # Overbought -> Sell signal
        score += 0
    elif last['rsi'] > 50:
        score += 10
    ...
    # 5. Volume Confirmation (15 poin)
    if last['volume'] > last['volume_ma20'] * 1.2:
        score += 10  # Volume tinggi konfirmasi
    if last['close'] > prev['close'] and last['volume'] > prev['volume']:
        score += 5   # Volume naik saat harga naik
        
    # 6. Support/Resistance (10 poin)
    if last['close'] <= last['support'] * 1.02:
        score += 10  # Dekat support -> Buy
    elif last['close'] >= last['resistance'] * 0.98:
        score += 0   # Dekat resistance -> Sell
    else:
        score += 5
        
    return min(100, max(0, score))  # Batasi 0-100
...
def multi_timeframe_analysis(symbol):
    ...
    weights = {
        "5m": 0.10,   # bobot kecil untuk timeframe kecil
        "15m": 0.15,
        "30m": 0.20,
        "1h": 0.25,
        "1d": 0.30    # bobot besar untuk timeframe besar
    }
